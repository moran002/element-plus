...
import static com.moran.utils.vo.Area.ID_LIST;
...
public class AreaUtils {
    ...
    public static Area parseArea(String pathStr) {
        ...
        for (String path : paths) {
            if (area == null) {
                area = areas.values().stream()
                        .filter(item -> item.getName().equals(path))
                        .findFirst().orElse(null);
            } else {
                area = area.getChildren().stream()
                        .filter(item -> item.getName().equals(path))
                        .findFirst().orElse(null);
            }
        }
    ...
    public static String format(Integer id, String separator) {
        ...
        for (int i = 0; i < AreaTypeEnum.values().length; i++) { // 避免死循环
            ...
            // “递归”父节点
            area = area.getParent();
            if (area == null || ID_LIST.contains(area.getId())) { // 跳过父节点为中国的情况
                break;
            }
