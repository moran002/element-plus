 ...
 *
 * @param item 传入的组件属性
 * @param formModel 表单数据模型，用于传递上下文信息
 * @returns 默认添加 clearable 属性
 */
export const setComponentProps = (item: FormSchema, formModel?: Recordable): Recordable => {
  ...
  // 处理函数类型的componentProps
  let resolvedComponentProps = item.componentProps
  if (isFunction(resolvedComponentProps)) {
    resolvedComponentProps = resolvedComponentProps(formModel)
  }
